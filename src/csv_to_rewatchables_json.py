...
input_csv = 'src/rewatchables.csv'
output_json = 'src/rewatchables.json'

with open(input_csv, newline='', encoding='utf-8') as csvfile:
    reader = csv.reader(csvfile)
    titles = [row[0].strip() for row in reader if row and row[0].strip() and not row[0].startswith('Movie Title')]
